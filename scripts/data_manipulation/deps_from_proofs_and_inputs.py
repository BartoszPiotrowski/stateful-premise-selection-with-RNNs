...
def deps_from_input(file_with_input):
    '''
    Extracts dependecies (axioms/premises) from a given file being
    input for E prover. The order of extracted premises is the order of
    appearance of the premises in the file.
    '''
    with open(file_with_input, 'r') as f:
        lines = f.read().splitlines()
    premieses = []
    conjecture = None
    for l in lines:
        l = l.replace(' ', '')
        name = l.split(',')[0].split('(')[1]
        formula_type = l.split(',')[1]
        if formula_type == 'conjecture':
            assert conjecture == None # it can only be one conjecture
            conjecture = name
        elif formula_type == 'axiom':
            premieses.append(name)
        else:
            raise ValueError
    assert conjecture
    assert premieses
    return conjecture, premieses


if __name__ == '__main__':
    proof_dir = sys.argv[1]
    for file_name in os.listdir(proof_dir):
        if '.E_output' in file_name:
            input_name = file_name.split('.')[0] + '.E_input'
            output_name = file_name
            conjecture_premises_proof = deps_from_proof(
                os.path.join(proof_dir, output_name))
            if conjecture_premises_proof:
                conjecture_proof, premises_proof = conjecture_premises_proof
                conjecture_input, premises_input = deps_from_input(
                    os.path.join(proof_dir, input_name))
                assert conjecture_input == conjecture_proof
                assert set(premises_proof) <= set(premises_input)
                print(f"{conjecture_proof}:{' '.join(premises_input)}#{' '.join(premises_proof)}")
